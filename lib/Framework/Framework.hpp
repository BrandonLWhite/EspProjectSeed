...
private:
    ...
    */
    ...
    {
        for(auto& appApi: getApi()) 
        {
            auto function = appApi.Function;
            auto path = "/app/" + appApi.Path;

            auto* handler = new AsyncCallbackJsonWebHandler(path.c_str(), [this, function](AsyncWebServerRequest *request, JsonVariant &json) {
...
public:
    ...
    void setup()
    {
        ...
        server.on("/wifi/scan", [this](AsyncWebServerRequest *request) { handleWifiScan(request); });
        server.on("/wifisave", [this](AsyncWebServerRequest *request) { handleWifiSave(request); });
        server.on("/wifi/status", [this](AsyncWebServerRequest *request) { handleWifiStatus(request); });
        server.on("/wifi/forget", [this](AsyncWebServerRequest *request) { handleWifiForget(request); });
        ...
        StatusUpdated.connect([this](const JsonObject & json)
        {
